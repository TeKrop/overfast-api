...
from app.adapters.blizzard.parsers.utils import parse_html_root, validate_response_status
...
def _parse_summary(root_tag: "LexborNode", player_summary: dict | None) -> dict:
    ...
    return {
        ...
        "avatar": (
            ...
            or summary_div.css_first("img.Profile-player--portrait").attributes.get("src")
        ),
...
def filter_all_stats_data(
    ...
    for platform_key, platform_data in stats_data.items():
        ...
        filtered_data[platform_key] = {
            gamemode_key: (
                gamemode_data if gamemode_key == gamemode else None
            )
